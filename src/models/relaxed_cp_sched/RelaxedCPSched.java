...
public class RelaxedCPSched {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < data.numjob; i++) {
            JobRSP newjob = new JobRSP(data.jobname[i], data.jobpartfamily[i], i, data.sj[i], data.dj[i], data.jobsteps[i], data.jobsteptimes[i]);
            jobobjs.add(newjob);
        }
        ArrayList<Tool> bottomobjs = new ArrayList<>();
        for (int i = 0; i < data.numbottom; i++) {
            Tool newtool = new Tool(data.bottomname[i], data.bottomcap[i], data.bottomqty[i], data.bottomsize[i], data.bottommin[i]);
        ...
        try {
            ...
            for (JobRSP j : jobobjs) {
                ...
                for (int i = 0; i < data.numjobtotool; i++) {
                    if (data.jobtotools[i][0].equals(j.name())) {
                        possibletoolnames.add(data.jobtotools[i][2]);
                    }
                }
                for (Tool b : bottomobjs) {
            ...
            for (JobRSP j : jobobjs) {
                for (int i = 0; i < data.numbatchtomachine; i++) {
                    if (data.batchtomachine[i][1].equals(j.steps()[0])) {
                        for (Machine m : machineobjs) {
            ...
            for (JobRSP j : jobobjs) {
                for (int i = 0; i < data.numbatchtomachine; i++) {
                    if (data.batchtomachine[i][1].equals(j.steps()[1])) {
                        for (Machine m : machineobjs) {
                            if (m.name().equals(data.batchtomachine[i][0])) {
            ...
            for (JobRSP j : jobobjs) {
                for (int i = 0; i < data.numbatchtomachine; i++) {
                    if (data.batchtomachine[i][1].equals(j.steps()[3])) {
                        for (Machine m : machineobjs) {
                            if (m.name().equals(data.batchtomachine[i][0])) {
            ...
            for (JobRSP j : jobobjs) {
                for (int i = 0; i < data.numskilltomachine; i++) {
